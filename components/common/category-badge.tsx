...
const badgeVariants = cva(
  "inline-flex items-center rounded-sm border px-1 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
        default:
          "border-transparent bg-secondary text-neutral-foreground hover:bg-primary/80",
        secondary:
...
function CategoryBadge({ className, variant, ...props }: BadgeProps) {
  return (
    <div className={cn(badgeVariants({ variant }), className)} {...props}>
      <div className="w-1 h-4 rounded-md mr-2 bg-primary" />
      {props.children}
    </div>
  );
}
